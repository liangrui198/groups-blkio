...
assign_processes() {
    ...
    if [[ "$enable_service" == "nm" || "$enable_service" == "all" ]]; then
      ...
      if [ ! -z "$NM_PID" ]; then
          ...
          echo $NM_PID > /sys/fs/cgroup/blkio/blk_hadoop/nodemanager/cgroup.procs
      fi
    fi
    # DataNode进程
    if [[ "$enable_service" == "dn" || "$enable_service" == "all" ]]; then
      ...
      if [ ! -z "$DN_WORKER_PID" ]; then
          ...
          echo $DN_WORKER_PID > /sys/fs/cgroup/blkio/blk_hadoop/datanode/cgroup.procs
      else
    ...
    if [[ "$enable_service" == "shuffle" || "$enable_service" == "all" ]]; then
      ...
      if [ ! -z "$SHUFFLE_PID" ]; then
          echo $SHUFFLE_PID > /sys/fs/cgroup/blkio/blk_hadoop/shuffle/cgroup.procs
      fi
    fi
    echo "进程分配完成"
}
